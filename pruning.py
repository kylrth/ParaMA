...
def get_suffix_type_score(affix_tuples):
    ...
    for affix_tuple, root_list in affix_tuples.items():
        for affix in affix_tuple:
            if affix in affix_type_dict: affix_type_dict[affix] += len(root_list)
...
def prune_affix_tuple(affix_tuple, affix_tuple_dict, affix_type_score):
    ...
    if affix_tuple in affix_tuple_dict: return affix_tuple
    
    # if affix_tuple only has one element, prune the whole paradigm.
    if len(affix_tuple) == 1: return tuple()
    ...
    for affix_tuple in affix_tuple_dict:
        ...
        if len(satisfied_affix) < 1: continue
...
def prune_paradigms(paradigm_dict, reliable_affix_tuples, affix_type_score, single_affix_tuples, tokens_prob_segs_dict, word_dict, exclude_unreliable):
